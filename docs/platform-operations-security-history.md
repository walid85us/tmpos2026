...
#### Post-replacement corrections (presentation-only — all ACCEPTED)

A sequence of scan-level table and Manage-drawer refinements over the accepted Tenant Web Address module. Every correction is presentation/wording-only: no model/storage/permission/audit/raw-status changes, the no-drift contract (flat `webAddresses` array + `matchesWebAddressFilter` predicate drives all cards/saved-view counts) is preserved, and `?domain=`/`?status=` deep-linking, the Add flow, and `view_domains`/`manage_domain_lifecycle` gating are untouched.

-   **Table simplification (post-QA #7):** trimmed the web-address list to scan-level columns; removed the full **Main App URL** column and per-link URL cells. Customer Links became a compact "N links available" + short chips (Portal/Book/Track/Pay/Mail-In), with full URLs / copy / open retained only in the selected overview.
-   **List grouping → one row per tenant/platform web address:** a derived `tableRows` grouping memo over the filtered `visible` set collapses a tenant's platform-subdomain record and its sibling external/redirect record into a single visual row (Pass 1 = one row per platform web address; Pass 2 = attach externals to that tenant's first platform row, or group external-only tenants into one row; two distinct platform web addresses stay two rows). The grouping is **table-only** — cards/saved-view counts still derive from the flat `webAddresses` array.
-   **Final minimal table:** reduced to four columns — **Tenant · Platform Web Address (inline copy icon) · Status · Actions**. The Customer Links and External Website columns were removed (details live in the overview); the action button was renamed **"Open" → "Manage"**. The unused `CUSTOMER_LINK_SHORT` map was deleted. Because the External Website column is gone, the selected overview now also lists the tenant's sibling external/redirect records (`selectedSiblingExternals`, derived from the flat `webAddresses` array), each with a **Manage** button, so no grouped external record is orphaned.
-   **Manage Drawer UX polish:** the selected slide-over (`WebAddressOverview`) was reorganized into a **Tenant Web Address Workspace** — header (tenant + platform web address with inline copy + kind/status chips, **Copy Link** + disabled **Open Site (Future)**), **Customer Access Points** (Main App + Portal/Book/Track/Pay/Mail-In via a new `AccessPointRow` — name · path preview · truthful status · copy · open-if-safe), **External Presence**, **Configuration Progress** (setup checklist), and **Recent Activity**. The gated/audited lifecycle Manage controls and the collapsed read-only Advanced Custom Domain Support reference panel were preserved; old `CustomerLinkRow` replaced by `AccessPointRow`.
-   **Final Drawer product-language cleanup:** the drawer's last domain-control-panel vocabulary was repointed to Tenant Web Address language without touching the model/storage/permissions/audit or raw status values. The Manage status select exposes only operator statuses **Active / Needs Setup** (via `MANAGE_STATUS_OPTIONS` + a `manageSelectValue` helper that maps any non-`verified` raw status down to "Needs Setup"); the `disabled` state stays in its separate read-only "Disabled" box; button "Mark Pending" → "Mark Needs Setup"; raw `RAW_STATUS_LABELS` is kept only for the advanced "Raw status" deep-link filter chip. Recent Activity uses a display-only `ACTIVITY_LABELS`/`activityLabel()` map (raw audit keys + legacy seeded history labels → "Platform/Web Address Created", "Web Address Status Changed/Disabled/Re-enabled", "External Website Recorded/Reviewed", "Security State Updated"); audit storage and event IDs are untouched. Metadata footer "Verified" → "Status reviewed", "Last checked" → "Last reviewed". The **Advanced Custom Domain Support** panel was de-technicalised — the DNS / SSL/TLS / Email DNS / Security readiness grid (and the `deriveDomainControlPanelOverview`/`AdvCell`/ADV label-map usages in `DomainsPage.tsx`) were removed in favor of support-assisted explanation copy plus the existing truth labels; the gated, audited manual SSL-state control is kept and the M0–M2 helper foundation remains intact in `platformOpsDomainModel.ts`/`platformOpsDomains.ts`. Configuration Progress completed items no longer use strikethrough (green check + readable muted text). Typecheck stable at the 12 pre-existing baseline errors (DomainsPage clean). **Accepted.**

## Phase 1.2 — Final Closeout (documentation/status alignment only)

Phase 1.2 (Domains + Platform Settings Maturity) is **finalized/accepted**. Closeout was documentation/status-alignment only — no code, behavior, permission, model, or route changes.

**Final decision:**

-   The earlier registrar/DNS "Domains" control-panel direction (and the rejected Phase 1.2E control-panel UX) is **superseded**. The accepted replacement is **Tenant Web Address**, now the correct SaaS product direction: a platform-managed tenant URL (`{tenant.subdomain}.repairplatform.com`), customer-facing access links, external-website / redirect guidance, and custom-domain support positioned as **future / support-assisted**.
-   Full DNS/SSL/registrar control-panel work is **not active**; the advanced DNS/SSL/registrar helpers (`platformOpsDomainModel.ts` / `platformOpsDomains.ts`, the M0 model, and the `tenant_domains_v1` store) remain **dormant/retained** for future support-assisted custom-domain work.
-   The **Platform Settings Control Center is accepted as-is for now** — truthful (nothing enforced at runtime; localStorage-only persistence; future capabilities clearly badged), correctly gated (`platform_settings` view / `edit_platform_settings` edit), and audited (one row per group save). Deeper implementation is deferred.

**Deferred / future items recorded:**

-   *Tenant Web Address:* customer-facing route activation/readiness; deeper integration with tenant provisioning; staff/owner onboarding linkage; public portal/booking/tracking/payment route finalization; support-assisted custom-domain support; real DNS / custom-domain verification only in a future enterprise/support phase.
-   *Platform Settings:* backend configuration service; runtime enforcement; SSO/SCIM; notification engine; provider integrations; approval-workflow engine; compliance-evidence automation; server-side policy enforcement.
...
Phase 1.3 Milestone 0 is an **access-inventory / documentation** milestone — no code, behavior, permission, resolver, route, model, mock-data, UI, or server changes. It establishes the authoritative current platform permission/RBAC picture (11 feature groups, 71 sub-permissions, 22 sensitive, 5 platform roles, the resolver order, and the UI-only enforcement reality) and classifies future server-side enforcement needs into Tier 0–3 before any governance implementation (Milestones 1–5) begins. The full inventory lives in the focused artifact [`docs/phase-1.3-platform-access-inventory.md`](phase-1.3-platform-access-inventory.md).

## Phase 1.3 — Milestone 1: Platform Team Governance Model (additive, non-enforcing)

Phase 1.3 Milestone 1 adds an **additive, non-enforcing governance model** — a single pure module (`src/owner/platformTeamGovernance.ts`) of types, constants, and deterministic helper logic for later milestones, plus its narrative doc [`docs/phase-1.3-platform-team-governance-model.md`](phase-1.3-platform-team-governance-model.md). It defines the current 5-role catalog (System Owner remains system-protected, never downgradeable), governance categories, a 14-entry privileged-action catalog mapped to the Milestone 0 server-enforcement tiers, and future-use temporary-access / access-review types with **derived/lazy** status helpers. **No current permission behavior changed**: the resolver, thresholds, defaults, and dependency auto-sync are untouched, the module is consumed nowhere yet, and there is no UI, persistence, workflow, scheduler, or automatic expiration. All helper outputs are advisory labels only. Milestones 2–5 remain **not started**.

## Phase 1.3 — Milestone 2: Platform Team Directory + Role Matrix UI (UI maturity, non-enforcing)

Phase 1.3 Milestone 2 matures the Team Management area (`src/owner/TeamManagementPage.tsx`) to **consume** the Milestone 1 advisory governance model for **display only**, with its narrative doc [`docs/phase-1.3-milestone-2-team-directory-role-matrix-ui.md`](phase-1.3-milestone-2-team-directory-role-matrix-ui.md). The Platform Team Directory now shows per-member governance posture (risk, recommended review cadence, system-protected, future access-review inclusion, future temporary-elevation eligibility, a standing *UI-Gated Today* badge) and a truthful **last-activity** column with a “not recorded” fallback; the Roles tab gains a read-only **Platform Role Catalog — Governance** section for the five current roles plus a clearly separated **Future / Deferred Role Concepts** area (Read-only Auditor, Platform Admin — documented concepts only, not active/assignable); the **Global Permissions Matrix** and **Effective Access Preview** are preserved, gaining only advisory governance labels and a clarified truth banner; and a new header notice states current platform access is UI/client-gated while server-side enforcement, automated access review, and temporary access are future/deferred. **No permission behavior changed** — resolver, thresholds, defaults, dependency auto-sync, role assignment, routes, server, and Firestore rules are all untouched; the only addition is display-only UI consuming existing helpers (with `lastActiveAt`/`invitedAt` carried through as optional display fields — no new store). `tsc --noEmit` stays at the pre-existing 12-error baseline (none in the touched files). Milestones 3–5 remain **not started**.

## Phase 1.3 — Milestone 3: Temporary Access / PIM Foundation (local, advisory, non-enforcing)

Phase 1.3 Milestone 3 turns the Milestone 1 advisory `TemporaryAccessGrant` types + derived/lazy status helpers into a usable **request → approve/grant → revoke** workflow (plus **deny** and **cancel**) inside the existing Team Management UI, with its narrative doc [`docs/phase-1.3-milestone-3-temporary-access-pim-foundation.md`](phase-1.3-milestone-3-temporary-access-pim-foundation.md). A new pure module (`src/owner/platformTemporaryAccess.ts`) provides a **session-scoped store** (`sessionStorage` key `platform_temporary_access_v1` + a `platform_temporary_access:changed` event) and **reason-required lifecycle transition helpers** (`createTemporaryAccessRequest`, `approveTemporaryAccess`, `denyTemporaryAccess`, `revokeTemporaryAccess`, `cancelTemporaryAccess`) that validate allowed transitions and append a reason-captured event to each grant's append-only `history[]`; `StoredTemporaryAccessGrant` **extends** the M1 type (kept pristine) with `elevatedRoleLabel`/`requestedDurationMs`/`history` only. The new **Temporary Access** tab shows single-source derived-status summary counts, a grant table (subject, advisory elevation target, time-box window, derived status, per-grant trail), a Request modal, reason-required approve/deny/revoke/cancel confirm modals, and a manual **Refresh Status** button. **Truthful limitations:** granting temporary access does **not** change real permissions (the module never touches `explainAccessDecision`/`platform_permissions_v1`, role defaults, thresholds, or dependency auto-sync — the elevation target is an advisory label); expiry is **derived/lazy** (clock re-anchored to approval time; stored status never auto-mutated; **no scheduler, no automatic escalation, no automatic revocation**); persistence is session-scoped (not durable backend state); System Owner stays system-protected (never an elevation subject or target); gating mirrors the rest of the page (System Owner only — **no new permission keys**). Five additive audit actions (`platform_temporary_access_requested/approved/denied/revoked/cancelled`, category `team`) were appended to `platformOpsAudit.ts`; each transition mirrors to both the in-page Activity Log and the shared Audit & Security log. `tsc --noEmit` remains at the pre-existing 12-error baseline (none in the touched files); a 38-check automated QA harness over the lifecycle logic passed 38/38. Milestones 4–5 remain **not started**; Phase 1.3 is **not complete**.

## Phase 1.3 — Milestone 4: Access Review + Sensitive Action Reason Capture (local, advisory, non-enforcing)

Phase 1.3 Milestone 4 turns the Milestone 1 advisory `AccessReviewRecord` type + access-review derivation helpers into a usable **create → record-outcome** access-review workflow, and adds **Sensitive Action Reason Capture** around existing sensitive governance actions, both inside the existing Team Management UI, with its narrative doc [`docs/phase-1.3-milestone-4-access-review-sensitive-action-reason-capture.md`](phase-1.3-milestone-4-access-review-sensitive-action-reason-capture.md). A new local/advisory module (`src/owner/platformAccessReview.ts`) provides a **session-scoped access-review store** (`sessionStorage` key `platform_access_review_v1` + a `platform_access_review:changed` event) and a parallel **sensitive-action reason log** (`platform_sensitive_action_reason_v1` + `platform_sensitive_action_reason:changed`); `StoredAccessReviewRecord` **extends** the M1 type (kept pristine) with `reviewedRoleLabel`/`reviewedRoleKnown`/`systemProtected`/`createdBy`/`recommendedCadenceLabel`/`history` only. **Reason-required transition helpers** — `createAccessReviewRecord` (validates subject / role / period / reviewer) and `completeAccessReview` (one of four terminal outcomes — `reviewed_no_change` / `reviewed_change_required` / `escalated` / `deferred` — reason required, never silent) — append a reason-captured event to each record's trail; `captureSensitiveActionReason` builds a reason-captured sensitive-action record (actor, M1 privileged-action category, reason, target, before/after, advisory label) without changing any permission result. The new **Access Review** tab shows single-source derived summary counts, a record table (subject, role + recommended cadence, period/reviewer, derived status, reviewed/updated, per-record trail), a **Create Review Record** modal, a duplicate-safe **Seed Team Reviews** action, a reason-required **Record Outcome** modal, a manual **Refresh Status** button, and a **Sensitive Action Reason Capture** panel. **Truthful limitations:** recording an outcome — including *change required* (labeled “Action required — advisory only; no permission change applied”) — does **not** change a member's role or permissions (the module never touches `explainAccessDecision`/`platform_permissions_v1`, defaults, thresholds, or dependency auto-sync); `overdue`/stale is a **derived label** via the M1 `isAccessReviewOverdue` (only an aged `pending` review ages into overdue; terminal/explicit states never auto-flip; **no scheduler, no automatic reminders, no automatic review completion, no automatic revocation**); persistence is session-scoped (not durable backend state); System Owner is in-scope but **system-protected** (rows show System Protected / Review Carefully / No Automatic Downgrade / No Permission Change Applied — never downgraded); gating mirrors the rest of the page (System Owner only — **no new permission keys**). Sensitive-action reason capture is wired to access-review outcomes and to the Global Permissions Matrix **Reset to Defaults** action (which gained a required reason field; its reset behavior and existing `platform_permissions_reset` audit row are unchanged); per-cell matrix edits, member edits, and M3 temporary-access actions keep their existing trails and are intentionally not re-gated (documented limitation). Six additive audit actions (`access_review_created`/`access_review_completed`/`access_review_change_required`/`access_review_escalated`/`access_review_deferred`/`sensitive_action_reason_captured`, category `team`) were appended to `platformOpsAudit.ts`; no existing action or severity changed. The Milestone 1 governance model (`platformTeamGovernance.ts`) is consumed but **unchanged**; `platformPermissionsConfig.ts`, `platformTemporaryAccess.ts`, `accessConfig.ts`, and `server/index.ts` have **no diff**. These records are **not compliance certification evidence**. `tsc --noEmit` remains at the pre-existing 12-error baseline (none in the touched files); a 49-check automated QA harness over the access-review + reason-capture logic passed 49/49 (temporary; removed after QA). Milestone 5 remains **not started**; Phase 1.3 is **not complete**.

## Phase 1.3 — Milestone 5: Command Center + Audit Integration + Documentation Closeout (visibility/closeout, local, advisory, non-enforcing)

Phase 1.3 Milestone 5 makes the M3/M4 governance work **visible and coherent** across the existing owner monitoring surfaces without adding enforcement, with its narrative doc [`docs/phase-1.3-milestone-5-command-center-audit-closeout.md`](phase-1.3-milestone-5-command-center-audit-closeout.md). **Command Center** (`src/owner/CommandCenterPage.tsx`) gains a read-only **Platform Team Governance** section — gated by the **existing** `view_command_center` page-overview permission (**no new permission key**) — that reads the M3/M4 session stores (read-only) and renders single-source derived-count cards: Temporary Access (Pending Requests / Active Advisory Grants / Expired Advisory Grants / Total via `summarizeTemporaryAccess`), Access Reviews (Pending / Overdue-derived / Escalated / Change Required via `summarizeAccessReviews`), and Sensitive Actions — Captured Reasons (`readSensitiveActionReasons().length`), each with a truthful empty state ("No local advisory … in this session."), a standing advisory notice ("Phase 1.3 governance signals are local/advisory … not … server-side enforcement, automated compliance evidence, or production PAM/IAM monitoring."), a future-state footer ("Server-side enforcement remains future/deferred … not production compliance evidence"), and cross-links to Team Management and the Audit governance lens. **Audit Investigation** (`src/owner/platformOpsInvestigation.ts`) gains one additive **Governance Activity (Advisory)** lens (`AuditLensId` member + `AUDIT_INVESTIGATION_LENSES` entry + a deterministic substring matcher over the M3/M4 action names) that the Audit & Security page renders automatically (it already maps over the lens list); its count and list share one predicate (no drift), and the Command Center cross-link deep-links `?lens=governance_advisory` through the page's existing visible/clearable lens handler. **Truthful limitations:** the M3/M4 `team`-category audit events were already auditable (All Events / `team` category) — M5 only adds a grouped lens and the monitoring-level cards; every count derives from the same store/helper the Team Management governance tabs use; derived expired/overdue recompute at load/Refresh only (**no scheduler, no background worker, no automatic revocation/review**); **no real backend enforcement, server middleware, or Firestore rule was added**, and these are **not production compliance evidence**. Non-regression: the resolver, `platformPermissionMeets`/`explainAccessDecision`/`hasPlatformPermission`/`hasEffectiveFeatureAccess`/`hasSectionAccess`/`hasActionAccess`/`AccessContext.canAccess`, role defaults, thresholds, and dependency auto-sync are untouched; `platformTemporaryAccess.ts`, `platformAccessReview.ts`, `platformTeamGovernance.ts`, `platformPermissionsConfig.ts`, `accessConfig.ts`, `server/index.ts`, and the Firestore rules have no behavior change (M3/M4 modules imported read-only; `platformOpsAudit.ts` was **not** modified in M5); existing Command Center and Audit Investigation behavior is preserved. `tsc --noEmit` remains at the pre-existing 12-error baseline (none in the touched files); a 24-check automated QA harness over the new governance-lens predicate (M3/M4 action matching, no-drift count, fallback contract) passed 24/24 (temporary; removed after QA). **Phase 1.3 is functionally complete only after this implementation report, validation, automated QA/static verification, manual QA acceptance, and the final GitHub backup — Milestone 5 implementation does not mark Phase 1.3 complete.**

### Milestone 5 — Correction Pass: Governance Permission Matrix Integration

A post-implementation correction (manual QA gate) integrated the M3–M5 governance capabilities into the **Platform Global Permissions Matrix** so the System Owner controls, per platform role, who can **view** or **manage** them — replacing the prior hardcoded System-Owner gating. **Seven additive sub-permissions** were added to `platformPermissionsConfig.ts`: `view_temporary_access` / `manage_temporary_access` (sensitive) / `view_access_reviews` / `manage_access_reviews` (sensitive) / `capture_sensitive_action_reasons` (sensitive) under **team_management**, `view_governance_signals` under **command_center**, and `view_governance_audit_lens` under **audit_security**, each with a label, plain-English description, threshold (view for the four view perms; manage for the three sensitive ones), and `sensitive` flag where appropriate; plus **five additive dependency entries** (`manage_temporary_access → view_temporary_access`, `manage_access_reviews → view_access_reviews`, `capture_sensitive_action_reasons → view_access_reviews`, `view_governance_signals → view_command_center`, `view_governance_audit_lens → view_audit_security`) that reuse the **existing** direction-aware auto-sync. **No resolver logic changed** — `platformPermissionMeets`, `explainAccessDecision`, `hasPlatformPermission`, `hasEffectiveFeatureAccess`, `getPlatformSubPermissionLevel`, role feature-defaults, thresholds, and dependency auto-sync are all unchanged; only additive catalog/dependency **data** and UI gating that **reads** the unchanged resolver were added. Gating was applied in `TeamManagementPage.tsx` (Temporary Access + Access Review tabs gated by the `view_*` perms with a defensive no-access panel and view-only notes; lifecycle/outcome/create/seed actions gated by the `manage_*` perms; the sensitive-action reason log gated by `capture_sensitive_action_reasons`), `CommandCenterPage.tsx` (governance section requires `view_command_center` **and** `view_governance_signals`; Audit cross-link requires `view_governance_audit_lens`), and `AuditSecurityPage.tsx` (governance lens card hidden, `?lens=governance_advisory` deep-link ignored, and active lens falls back to "All Events" without `view_governance_audit_lens`). **System Owner resolves to Full Access for every key and stays fully allowed/protected**; the matrix Reset-to-Defaults keeps its existing System-Owner-only gate (who can reset was **not** broadened; reset behavior unchanged). Default least-privilege posture (sub-permissions inherit the parent feature level): System Owner + Security/Audit Admin can view **and** manage governance and see the sensitive-action log; Operations / Support / Billing Admin are **view-only** (their view access follows their existing view-level access to the parent features and can be set to None per role by the System Owner). These controls remain **UI/client-gated only** — no server-side enforcement, Firestore rules, server middleware, scheduler, or worker were added; `server/index.ts`, the Firestore rules, and tenant/store `accessConfig.ts` have **no diff**. `tsc --noEmit` stays at the pre-existing **12-error baseline** (none in the touched files); a temporary 74-check QA harness (catalog placement, thresholds, sensitive flags, dependency entries, per-role resolution, System-Owner allow-all, and dependency auto-sync raise/cap) passed 74/74 and was removed. Phase 1.3 remains **not complete** (pending manual QA acceptance + final backup).

## Phase 1.3 — Final Closeout (documentation/status alignment only)

Phase 1.3 — **Platform Team Governance / Server-Side Access Enforcement Scope foundation** — is **COMPLETE / ACCEPTED / BACKED UP**. All milestones are accepted and backed up: M0 (Current Access Inventory), M1 (advisory governance model), M2 (Platform Team Directory + Role Matrix UI), M3 (Temporary Access / PIM Foundation), M4 (Access Review + Sensitive Action Reason Capture), and M5 (Command Center + Audit Integration + Documentation Closeout, including the correction pass). The **M3–M5 governance features are controlled through the Platform Global Permissions Matrix** (seven additive sub-permissions + dependency entries), so the System Owner decides — per platform role — who can view or manage temporary access, access reviews, sensitive-action reason capture, the Command Center governance signals, and the Audit governance lens. **Current enforcement remains UI/client-gated and local/advisory where applicable**: granting these matrix permissions controls who can see/operate the advisory governance surfaces, but no governance record changes a real role or permission, and the permission resolver, role defaults, thresholds, and dependency auto-sync are unchanged. **Server-side enforcement, Firestore security rules, SSO/SCIM, production PAM, scheduler-based / automatic access revocation, and compliance-evidence automation remain future/deferred** — these governance signals/records are **not** production compliance evidence. The latest backed-up GitHub checkpoint is `7ad9ed2595ff10f008414190542fdb4cfcf60bbe`. **No new phase has started.**
