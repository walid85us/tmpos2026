...
> **Status:** **RATIFIED — WORKING DIRECTION (PostgreSQL).** The *direction* (PostgreSQL as the production database) is accepted as the working direction. **As of the Product Owner Input Collection (2026-06-12 — see note below), Supabase is ratified as the final working/default production *provider direction* (managed PostgreSQL, US region), subject to selecting the paid tier (with PITR), exact region, and add-ons at implementation; Supabase Auth is the future production auth direction (migrate before production users; Firebase Auth for testing meanwhile).** Neon / Railway remain acceptable fallbacks; **Hostinger-VPS PostgreSQL is not preferred** for production financial/POS/audit data (managed backups required). The **exact tier/region/runtime, the detailed backup/restore plan, the residency confirmation, the secret manager, and the first implementation slice remain PROVISIONAL/pending.** **This record still triggers no migration, creates no schema, and changes no code; ratification here is of a direction, not an implementation.**
>
...
>
> **M5 update (2026-06-12, accepted/backed up):** [M5 — Provider Finalization Gate](phase-1.4-milestone-5-provider-finalization-gate.md) defines the **decision gate** that must be satisfied before this status may be escalated. **The final provider and final auth remain PROVISIONAL until the M5 gate is satisfied:** the product-owner input checklist (M5 §5) must be answered, a written backup/restore plan must meet a ratified RPO/RTO, region/residency + budget band + scale envelope must be accepted, and the matching per-provider go/no-go criteria (M5 §17–§23) must pass. **Supabase remains the preferred working candidate, not the final provider.** **No backend implementation may begin before the M5 gate inputs are answered** (M5 §24–§26). To finalize later, record the M5 §27 intake template here and follow M5 §28; do not escalate this status by assumption.
>
> **M6 note (2026-06-12, accepted/backed up):** [M6 — Backend & Persistence Readiness Closeout](phase-1.4-milestone-6-backend-persistence-readiness-closeout.md) closes out Phase 1.4 (docs/architecture only) and is the authoritative handoff. It **does not escalate or finalize** this record's status; the recommended next step is **Product Owner Input Collection** (record inputs here via the M5 §27 intake template) before any implementation.
>
> **Product Owner Input Collection update (2026-06-12, pending review):** the product owner answered the full M5 §5 input checklist and the gate was run — see [`phase-1.4-product-owner-input-collection.md`](phase-1.4-product-owner-input-collection.md). **Gate result:** **Supabase = GO → ratified as the final working/default production provider *direction*** (US region; tier with PITR/region/add-ons at implementation); **Supabase Auth = GO → future production auth direction** (migrate before production users; Firebase Auth for testing meanwhile, not permanent — I21=No); **Neon/Railway = acceptable fallbacks**; **Hostinger-VPS PostgreSQL = NO-GO/not preferred** for production financial/POS/audit data (managed backups required — I15=Yes). **Accepted targets:** managed backups + PITR, RPO ≤15min, RTO ≤4h, 30-day operational + 7-year financial/audit retention; budget ~$200/mo (flexible); Stripe+Square payment options with **no card data in the app DB**; local dev = Supabase CLI; schema tooling = SQL-first/Supabase migrations (ORM deferred). **No implementation is authorized:** the first implementation slice, the detailed backup/restore plan, the concrete secret manager, and the pre-go-live residency confirmation are still pending (POIC §17–§18). Supabase is **not implemented**; this is a direction ratification only.
