...
- **Milestone 4 (Access Review + Sensitive-Action Reason Capture):** `AccessReviewRecord` + the access-review helpers and `PRIVILEGED_ACTION_CATALOG` (`shouldRequireReasonForPrivilegedAction`, `shouldAuditPrivilegedAction`) define which actions warrant reason prompts and how stale access is flagged. **(Now adopted)** — Milestone 4 consumes `AccessReviewRecord`, `ACCESS_REVIEW_STATUS_LABEL`, `isAccessReviewOverdue`, and `getRecommendedReviewCadence` via the new local/advisory store `src/owner/platformAccessReview.ts` (session-scoped, reason-required outcomes, derived overdue, no scheduler, never applied to the resolver). See [`docs/phase-1.3-milestone-4-access-review-sensitive-action-reason-capture.md`](phase-1.3-milestone-4-access-review-sensitive-action-reason-capture.md). The types/helpers in this module remain **unchanged**.
- **Milestone 5 (Command Center / Audit integration):** `PRIVILEGED_ACTION_CATALOG` + `getFutureServerEnforcementRecommendation` provide consistent categories/labels for surfacing privileged-action signals. **(Now adopted as visibility only)** — Milestone 5 surfaces the M3/M4 local/advisory governance activity across the existing owner monitoring surfaces: a read-only **Platform Team Governance** signals section in the Command Center (derived counts over the M3/M4 session stores) and a **Governance Activity (Advisory)** lens in the Audit Investigation Center. A **correction pass** then integrated all M3–M5 governance capabilities into the **Platform Global Permissions Matrix** via seven additive sub-permissions (`view_/manage_temporary_access`, `view_/manage_access_reviews`, `capture_sensitive_action_reasons`, `view_governance_signals`, `view_governance_audit_lens`) with dependency entries — so the System Owner controls who can view/manage governance per role. **No resolver/`platformPermissionMeets`/`explainAccessDecision` logic changed; no role defaults, thresholds, or dependency auto-sync logic changed** (only additive catalog + dependency data and UI gating). No enforcement, scheduler, worker, server-side check, or compliance-evidence automation was added; these are not compliance certification evidence. See [`docs/phase-1.3-milestone-5-command-center-audit-closeout.md`](phase-1.3-milestone-5-command-center-audit-closeout.md). The types/helpers in this module remain **unchanged**.
