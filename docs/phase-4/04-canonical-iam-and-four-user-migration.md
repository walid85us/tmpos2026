...
**Named-grant-only (prevents silent SoD collapse):** `activate_payment_gateway`, `disconnect_payment_gateway`, `manage_payment_gateway_connections`, and `manage_payment_terminals` are satisfiable **only by their specific named grant** — they are **NOT** auto-conferred by a broad `payments`-domain `manage`/`full` level (nor by the Store Owner's blanket `full` except via the explicit role-policy grant). Otherwise the level ordering (`full` is the top of the store family, [§3](#3-permission-level-reconciliation-required--gap-11)) would let any `payments: full` holder silently satisfy both the config and operation sub-permissions, collapsing the SoD and the "generic manager not auto-authorized" guarantee. These sub-permissions are covered by the **[§3](#3-permission-level-reconciliation-required--gap-11) family-aware contract** ([ADR-19](./10-architecture-decision-records.md)).

## 3. Permission-level reconciliation (**required — GAP-11**)

**Status (M5-GAP11-P5):** closed at repository level by family-aware permission semantics (ADR-19). Two separate, authoritative orderings now coexist for tenant/store and platform contexts.

Before M5-GAP11-P5 there were **two conflicting orderings** in the codebase, each behind its own comparator:
...
**Canonical decision (M5-GAP11-P5):** instead of a global ordering, implement **one family-aware permission contract** that recognizes two authoritative orderings. Tenant/store operations use `none → view → create → edit → manage → approve → full`; platform governance uses `none → view → create → edit → approve → manage → full`. Every authorization comparison is family-aware — it names the family (`meetsPermissionRequirement({family,level},{family,level})`, `permissionLevelRank(family, level)`), denies on unknown/malformed family or level, and refuses cross-family comparison. The source is `src/authorization/permissionFamilies.ts` (`TENANT_STORE_ORDERING`, `PLATFORM_ORDERING`, `meetsFamilyLevel`, `meetsPermissionRequirement`, `permissionLevelRank`). Money actions (`approve_refunds`, `approve_return`, `approve_billing_actions`) are explicit editable boolean capabilities (`src/authorization/moneyCapabilities.ts`) — never inferred from an ordering or a level. The D2 grant table is reclassified as the built-in money-grant defaults (`BUILT_IN_MONEY_GRANT_DEFAULTS`, the same values); the thirteen D3 compatibility pins are retired from active evaluation. Both, the grant diff and the shadow comparator (`gap11ShadowComparator.ts`, diagnostic only, wired into no decision path) are kept as history, with the evidence artifact `docs/phase-4/evidence/gap11-ordering-flip-grant-diff.md` frozen at commit 84fa74e9.

Why not the global ordering: M5-GAP11-P4 showed that one ordering plus D2 and D3 cannot preserve owner-edited and custom tenant roles in the deployed client (an owner-edited `inventory: approve` stops satisfying `inventory: manage`; a manager whose Returns level is edited to `approve` loses `approve_return` and `complete_return_disposition`), and pins keyed to default role states cannot follow an edit. Keeping each family's own ordering keeps every non-money store answer in every reachable role state, and moving money approval to explicit grants removes the only reason the ordering mattered for money.

What the family contract guarantees:

1. **Named family:** the public comparators (`meetsPermissionLevel` for the store family, `platformPermissionMeets` for the platform family) keep their answers and delegate to the family core; every other comparison passes its family literally.
2. **Deny-by-default on unknowns:** an unknown or malformed family, level, capability or grant value denies; a comparison across families is refused.
3. **Money actions via explicit grants:** built-in roles start from the defaults (refund and return approval: `manager`, `store_owner`; billing approval: `billing_admin`, `system_owner`); the owner's `true` grants and `false` revokes; custom roles start without a grant; no level (`approve`, `manage`, `full`) grants one; plan, entitlement, parent-module minimum, read-only, identity, tenant, store and scope gates still deny on top of a `true` grant. The two refund-approval forms (the `approve_refunds` sub-permission and the POS supervisor check) are one capability: Refunds at View or above plus the grant. The platform System Owner holds no store role and is denied store refund and return approval.
4. **No provenance-dependent semantics:** default, owner-edited and custom roles are decided by the same rule.
5. **Intended-outcome tests:** 49 pairs in each family; the money capabilities; the production-reachable role states regenerated from code (259 built-in-default decisions, 1,517 single-area edits, 2,598 sub-permission toggles, 504 custom-role decisions) with no non-money change and six enumerated money changes, each an owner decision.

**Permission families and levels:** two authoritative families — store `none, view, create, edit, manage, approve, full`; platform `none, view, create, edit, approve, manage, full` — each exported once from the canonical source. Preserved platform feature groups (11) and their sensitive sub-permissions. No inferred level, no fallback to a single global rank. The DEV-only route catalogue (`evaluateCanonicalPermission`) still refuses tenant and store scope until owner decision D1 defines a specific route and its product rules.

**Superseded decision (M5 phase (i), [ADR-04](./10-architecture-decision-records.md); rejected by M5-GAP11-P4 and replaced by M5-GAP11-P5 — kept as history):** adopt **one** ordering — `none → view → create → edit → approve → manage → full` (approve below manage), matching the platform ordering and the intended semantics ("manage" ⊇ "approve"). A single **shared permission catalog** (the "declared, NOT built" `SHARED_PERMISSION_CATALOG_TARGET`) becomes the one source of truth for levels, domains, and sub-permissions.
...
1. **Re-pin, don't infer:** for each affected `approve`-gated sub-permission, set the explicit required level (re-pin `approve`-gated money actions to require `≥ manage` **or** an explicit per-role grant) so the ordering flip does not auto-expand it. *(Owner decision D2: the explicit per-role grant — carried in the candidate only, not cut over; [09](./09-roadmap-m0-m9.md), M5-GAP11-P2. Owner decision D3 adds thirteen ordering-compatibility pins, one for each tuple the flip would still change, each keeping today's answer — M5-GAP11-P3.)*
2. **Grant-diff report:** produce a before/after effective-grant diff for every (role × domain × action); no silent change ships.
3. **Owner approval:** the diff is reviewed and explicitly owner-approved before cutover. *(Owner decision D3: every change the flip makes is rejected, and the resulting zero-diff artifact is approved, conditionally on staying empty — M5-GAP11-P3; approval is not a cutover.)*
...
(All six safeguards execute in M5 phase (i); "phase" here is execution ordering within M5, not a new milestone.) *M5-GAP11-P5 disposition: #1 is met by explicit money grants instead of a re-pin; #2 and #3 stand as the historical record of the rejected cutover; #4 stands; #5 has no authority switch to guard; #6 is delivered as the intended-outcome tests above.*
