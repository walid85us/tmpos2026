...
- **Session, rate-limit and idempotency backing stores:** distributed rate limits (component 13) may use a shared cache (e.g. Redis); no store is chosen yet for server sessions and the revocation list (M4). Durable idempotency records and the transactional outbox are PostgreSQL state committed in the business transaction ([ADR-17](./10-architecture-decision-records.md)) — a cache is never their sole authority. The current in-memory/process-local stores do not survive restart or scale across instances (GAP-15) and are replaced here.
