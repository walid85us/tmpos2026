...
| **M7e** | Shipping | shipment, tracking_event, pickup, provider_config, sla_policy | M7b, M7d | Shipping ships **orders/returns**; requires the **provider egress boundary + encrypted credentials** (closes SSRF/secret gaps, GAP-03/06) and webhook signature verification (GAP-14). **Consistency dependency:** the Shipping provider credential handling is built to the **same encrypted credential-service + store-scoped isolation + verified-webhook standard as payments** ([05 §5.1](./05-canonical-data-ownership-and-api-db-contracts.md)) — no new milestone. **Starting point (Phase 4.0 M3):** the legacy DEV Shipping sidecar was **ELIMINATED, not migrated** — `server/index.ts` (22 unauthenticated routes incl. the `label-proxy` SSRF), `credential-store.ts` (the unauthenticated in-memory global credential store), `event-processor.ts` (webhook ingest/replay + unauthenticated durable filesystem writes), the three carrier adapters, and `server/types.ts` were deleted, so M7e rebuilds from the canonical contracts rather than porting that code. **Ownership:** shipping provider configuration is **STORE-owned** — configured in Store Settings by the Store Owner or another active store user holding the explicit canonical shipping-provider permission, scoped to one canonical tenant+store, never authorized merely because the caller is a System Owner (the System Owner governs the connector catalog only). |
