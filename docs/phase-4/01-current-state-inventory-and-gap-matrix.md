...
- ~~`POST /identity/resolve` — **NONE (caller auth deferred)** — **upserts `platform_identity`** with caller-supplied, unverified `authProviderUid` ⇒ **unauthenticated DB write / identity spoof**, gated only by a flag.~~ **DELETED in M3** (bounded JSON 404). Since M5-ID-P1 the only way to turn a provider reference into an app-owned actor is migration 007's `tmpos_identity.m5_resolve_principal` (corrected in M5-GAP11-P1-R1: an earlier revision said `tmpos_internal`; 007 creates its own schema), which takes a reference the authentication boundary has ALREADY verified, writes nothing, and hands the runtime role no privilege on `platform_identity`.
