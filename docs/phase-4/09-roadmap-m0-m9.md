...
**M6-IDEMPOT-P2 (not live) — the durable idempotency contract foundation:**
...
- **What remains.** No store is approved or composed. The authoritative implementation must commit the business mutation, the lease-checked completion, the audit record and the outbox record in one transaction; until it exists the crash window stands, and the contract guarantees one holder of an unexpired lease and the replay of a completed operation, never exactly-once execution. The **transactional outbox is the next M6 slice**; full audit coverage and credential rotation are untouched.
