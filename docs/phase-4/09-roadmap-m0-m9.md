...
**M6-RATE-P1 (uncommitted, not live) — trusted-proxy identity and the distributed rate-limit foundation:**
- **Client address.** One resolver (`server/runtime/clientAddress.ts`) decides who the client is:
  ...
  - IPv4 clients are limited per address and IPv6 clients per /64. NAT64 and 6to4 addresses count as the IPv4 address they carry, and IPv4-mapped IPv6 counts as IPv4.
...
- **What remains.** No distributed adapter is approved or composed, so **M6 is not complete**. The rate-limit half still needs an approved shared store, its adapter, and the deployment half of the proxy contract (details in [08](./08-production-gate-and-risk-register.md) G-IDEMPOT and G-UNAUTH). Durable idempotency, events/outbox, full audit coverage and credential rotation are untouched. M4's production blockers (the admission race, G-DBROLE, migration 005) are separate and unchanged.
