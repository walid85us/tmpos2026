...
- **Driver containment.** No released postgres.js fixes either connection-loss defect (DA-15), so the dependency stays at 3.4.9 and the store contains both in its own code: once a transaction's `begin` has settled, nothing more — no statement, ROLLBACK or COMMIT — is handed to the driver, and `createSupervisedPgClient` retires a pool after any failure its transaction body did not raise, so the next call opens on a fresh one. Nothing is retried; failures before COMMIT stay `unavailable`, an ambiguous COMMIT stays indeterminate, and a retired pool ends within a bounded grace. Child-process tests witness both defects in the pinned driver and prove the store survives each loss (M6-PG-24–26).
