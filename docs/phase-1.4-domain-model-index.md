...
> **Documentation only.** Compact companion to [`phase-1.4-milestone-2-durable-data-shape-domain-model.md`](phase-1.4-milestone-2-durable-data-shape-domain-model.md). Not a schema, not a migration, provider-agnostic. Phase 1.4 closeout: see [`phase-1.4-milestone-6-backend-persistence-readiness-closeout.md`](phase-1.4-milestone-6-backend-persistence-readiness-closeout.md). Provider/auth direction ratified (Supabase + Supabase Auth, not implemented): see [`phase-1.4-product-owner-input-collection.md`](phase-1.4-product-owner-input-collection.md). Legend: **Scope** P=platform / T=tenant / S=store / X=cross-scope. **Now** = current persistence (FS-RO Firestore-read / SS sessionStorage(advisory) / LS localStorage / MEM in-memory mock / STATIC config / DERIVED / dev-FILE / server-MEM). **Diff** = future migration difficulty. **Sens** = enforcement sensitivity. **Audit** = future audit requirement. ✱ = append-only candidate. **Order** = persist-first(1) / defer(D) per M2 §15–16.
