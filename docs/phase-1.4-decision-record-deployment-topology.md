...
>
> **Product Owner Input Collection update (2026-06-12, pending review):** the gate was run with product-owner inputs — see [`phase-1.4-product-owner-input-collection.md`](phase-1.4-product-owner-input-collection.md). **Now confirmed:** **Hostinger = static frontend/marketing/demo only, NOT production database** (I18); **Replit = development only** (I19); the managed PostgreSQL provider direction = **Supabase**, kept **off-host** (managed, not on the Hostinger host). **The server/API-tier requirement stays RATIFIED; the specific runtime remains PROVISIONAL** — narrowed to **Supabase Edge Functions vs an external API/serverless tier**, to be chosen at implementation. **No topology/runtime implementation is authorized;** `.replit`, build config, deployment target, and `server/index.ts` are unchanged.
