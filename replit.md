...
-   **Shipping Provider Configuration**: Centralized configuration system accessible from Settings → Shipping for EasyPost, Shippo, and ShipStation. Supports credential configuration (masked display), environment selection, connection testing, and active provider selection, with state persisted in `sessionStorage`. All configuration actions are gated by `manage_shipping_settings` sub-permission and respect preview mode.
