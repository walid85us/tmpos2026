...
-   **Commercial Controls**: Implements a system for managing add-ons and feature entitlements with commercial governance, tenant overrides, pricing rules, and a detailed audit trail.
    ...
    -   **QA Correction Pass (Parts A-F)**:
        ...
        -   *Part C — Permissions Impact (REMOVED in follow-up correction)*: An earlier iteration rendered an inline "Permissions Impact" mini-section beneath each Features-tab row. Per the follow-up QA correction this block was removed — the Tenant Features tab is a **commercial entitlement surface only** and must not duplicate role-permission detail. A single italic wayfinder line ("Manage role permissions in the Store Permissions Matrix.") appears once at the bottom of the features list. The Store Permissions Matrix remains the sole source of role / sub-permission detail.
        ...
        -   *Part F — Disabled-by-Owner correction*: The resolver now suppresses `feature_disabled_by_owner` when the plan does not include the feature in the first place — surfacing the truthful `disabled_by_plan` ("Not in Plan") instead, since the disabled flag is dormant. Rows that legitimately show `Disabled by Owner` (plan would include it + disabled override exists) expose a `Re-enable` action that removes the dormant disabled override and audits it. Concrete effect: Reports & Analytics on an `essential`-plan tenant displays as "Not in Plan" with truthful Trial / Paid Override actions, not as "Disabled by Owner".
    -   **Tenant Features Tab Information Architecture (locked)**:
        -   The Tenant Features tab answers only commercial-entitlement questions: Is this feature included by plan? Can the System Owner grant Trial / Paid Override? Is it currently Trial / Paid Override / Pending Payment / Revoked / Trial Expired?
        -   It must NOT render permission chips, locked sub-permission badges, or any "Permissions Impact" block.
        -   Trial / Paid Override actions are **independent** of Add-on Catalog linkage — any implemented feature not currently in the tenant's plan is eligible for fresh grants. An optional violet `Add-on` pill appears when an active paid override is linked to an active Add-on Catalog item.
        -   `Disabled by Owner` only renders when the plan would otherwise include the feature AND a disabled override exists; otherwise the row resolves to `Included by Plan`, `Not in Plan`, `Not Available`, or `In Development`.
        -   `Pending Payment` rows show only `Mark Paid` / `Cancel Invoice` / `View Invoice`. `Paid Override` rows with an open invoice show `Revoke` / `Mark Paid` / `View Invoice` plus an `Invoice Open` / `Invoice Overdue` secondary pill. Revoked rows show only `Re-trial` / `Re-grant Paid` (never alongside fresh grant buttons).
