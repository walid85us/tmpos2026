...
-   **CRM (Customers)**: Full CRM with enriched Customer type (tier, tags, notes, assets, custom fields, GDPR compliance, campaigner status, third-party billing). Customer list with multi-field search (name, email, phone, tags). Clean list rows without avatar placeholders — name and ID shown directly. Per-row action menu dropdown (View, Edit, Start Sale, Create Ticket) with outside-click dismiss. Edit Customer modal with all profile fields; loyalty tier/points fields gated by `loyalty_customer_edit` sub-permission (disabled with contextual warning — plan-locked vs permission-denied). Loyalty management panel in profile view with progress bar and tier-to-next tracking (feature-gated by `loyalty_management` plan feature via `canAccess`; edit/adjust actions gated by `loyalty_customer_edit` sub-permission). Loyalty Settings button in list header gated by `loyalty_settings_manage` sub-permission for program config (enable/disable, pointsPerDollar, tiers CRUD with privileges list per tier). When loyalty is plan-blocked, a locked indicator ("Loyalty — Upgrade Plan") is shown instead of the settings button. Plan gating takes precedence over store-side sub-permissions. Manual point adjustment with reason + history. Disabled program indicator (Program Paused badge, frozen points warning in edit modal). Tier dropdown in loyalty edit syncs with active tiers from `loyaltyConfig.tiers` dynamically. Duplicate detection on create matches phone, email, AND full name (case-insensitive, normalized whitespace); shows per-record Merge button + "Create Anyway" override. Clickable order rows in Orders tab open order detail overlay. Clickable invoice numbers in customer history open full invoice detail overlay modal. Start Sale / Create Ticket navigate to POS with customer context via location state. Customer profile header uses compact name+tier+points layout (no avatar placeholder).
-   **Invoices**: Full invoice lifecycle management connected to `StoreLocalState`. Create invoices with customer picker, dynamic line items (product/repair/service types), searchable inventory picker for product items (by name/SKU), service catalog grouped by category via optgroups, discount, auto tax (8%). Invoice detail as modal overlay (no page navigation). Invoice edit modal for unpaid invoices. Payment restricted to Cash + Card Terminal only; Card Terminal uses state machine flow (`idle → pending_terminal → confirmed/failed/cancelled`) with 3s simulated terminal confirmation and Cancel/Reject button available even after confirmation; closing payment modal resets terminal state (cancel-on-close behavior). Cash payments require explicit "Confirm Cash Received" confirmation step before applying. Invoice reopen feature: cash-paid invoices show Reopen button — green with open-lock icon for authorized users (`reopen_invoice` sub-permission), orange with locked icon + supervisor PIN authorization modal for denied users (mirrors POS expired warranty supervisor PIN pattern). Card-paid invoices do not show reopen (use refund flow instead). Invoice `statusHistory` tracks all status transitions (created, paid, partially_paid, reopened, reopened_supervisor, cancelled) with actor, timestamp, fromStatus/toStatus, and notes — displayed in reverse chronological order in invoice detail sidebar. Print pipeline uses dedicated hidden `#print-surface` div (fixed position, z-index 999999) rendered outside modal hierarchy — print output shows only the latest payment (not full history) for clean output. Print modal offers Full Page vs Receipt mode toggle. Full-page prints professional invoice with header, billing block, line items, totals, latest payment info, notes/terms. Receipt mode prints narrow 80mm thermal-receipt format with dashed separators and monospace font. Print CSS uses `position: fixed !important` with `@page :first` margins and `print-color-adjust: exact`. Print/Email/SMS/Online Pay Link action modals layered on detail view. Status filter and search. Stats dashboard (outstanding, paid today, overdue, recurring). Permission-gated create/edit via `checkPermission('invoices', ...)`. Shared `renderLineItemEditor` and `renderInvoiceFormModal` helpers for create/edit.
...
-   **Document Template Editor**: Full template editor in Settings with 5 document types (Invoice, Repair Ticket, Inventory Label, Sales Receipt, Price Estimate). Each template uses a macro/tag system (`{{storeName}}`, `{{customerName}}`, `{{lineItems}}`, etc.) with categorized tag sidebar panel. Source mode for direct HTML editing with tag insertion at cursor. Visual preview mode renders template with sample data, including conditional block support (`{{#if discount}}...{{/if}}`). Save/Reset/Close controls with unsaved-changes indicator. Modified templates show badge on Settings card. State managed in `StoreLocalState` (`documentTemplates`, `updateDocumentTemplate`, `resetDocumentTemplate`). Types: `DocumentTemplate`, `TemplateType`, `TemplateMacro` in `types.ts`. Component: `TemplateEditor.tsx`.
-   **Data Flow**: `StoreLocalState.tsx` manages shared local mock state for customers (enriched), stock items, held orders, completed orders (with `CompletedOrderItem` supporting `deviceName`, `imei`, `serialNumber` for repairs), refund records, warranty claims, POS operators, invoices, repair services, service categories, loyalty program config (with tier privileges), and document templates. Includes `findDuplicateCustomers()` helper (matches phone, email, and normalized full name). `LoyaltyTier` supports `privileges?: string[]` array. Plans/Features/AddOns data in PlansPage persists across navigation via `sessionStorage`. Seed completed orders include orders from Oct/Nov 2025 with expired warranty items for testability.
-   **Feature Gating & Navigation Derivation**: `canAccess()` uses a merge-based strategy for plan feature resolution. The static `planFeatures` baseline (in `accessConfig.ts`) defines the canonical feature set per plan. When `features_data` exists in sessionStorage (written by PlansPage), only features explicitly present in the featureMatrix are gated by their `planAvailability`; features NOT in the matrix pass through unchanged from the baseline. This prevents toggling one feature in Platform Command Center from corrupting unrelated sidebar modules. Normalizes `supply-chain` ↔ `supply_chain` and maps `starter` → `essential` plan key. Both `featureMatrix` format (`[{id, planAvailability}]`) and legacy `planEntry` format are supported with identical merge behavior. Plan-only features (e.g. `loyalty_management`, `suggestive_sales`, `refunds`) — those not in `PERMISSION_DOMAINS` — return `true` for all roles when the plan enables them; RBAC sub-permissions handle granular gating independently.
