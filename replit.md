...
-   **Shipping Center**: Centralized shipping module (`ShippingCenter.tsx`) for managing shipments across all source documents. Features: shipment list with status/source filters, 3-tab detail view (Overview/Tracking/Packages), create/edit modals, status transition workflow (Draft → Ready → Label Created → Packed → Dispatched → In Transit → Delivered), tracking event timeline, package management, carrier/service level configuration, and cost tracking. Types: `customer_delivery`, `repair_return`, `store_transfer`, `rma_outbound`, `rma_return`. Sources: `invoice`, `repair`, `transfer`, `rma`. Integrated with Invoices (action bar), Repairs (sidebar), and Supply Chain RMA (overview tab) via "Create Shipment" buttons that auto-populate shipment data from source documents. 8 sub-permissions (`create_shipment`, `edit_shipment_pre_dispatch`, `dispatch_shipment`, `update_tracking_events`, `cancel_shipment`, `view_shipping_costs`, `manage_shipping_settings`). Dual-gated: plan entitlement (`canAccess('shipping')`) + role permission (`checkSubPermission`). Available on Growth and Advanced plans.
