...
-   **Latest work**: Phase 1.2E **Domains Control Center UX Maturity — Milestone 4 (Default Baseline Manager + Change Review Center Polish)** — a focused presentation/navigation pass over the System Owner **Platform Settings** page. Adds (1) a **Customized Defaults** card in the Default Baseline workspace showing each field whose saved baseline differs from the built-in registry default (registry→baseline diff, group/risk, customized + high-risk-customized indicators, inline edit jump); (2) a consolidated **Change Review Center** modal (header button + Overview CTA) listing **every** unsaved change across all groups — settings and baseline — with label/key/group/old→new/risk/enforcement/impact/truth, derived from the **unfiltered** change sets so filters can never hide a pending change, still saving **per group** through the existing review/save handlers (high-risk ack + one audit row per group save preserved); and (3) a **Recent Configuration Changes** card reading mirrored audit rows (`readMirroredAuditRows()`, filtered to the two config actions, refreshed on `audit_logs:changed`) with a deep-link to the Audit Investigation Center. **Pure presentation/navigation** — storage keys (`platform_settings_v1`, `platform_settings_defaults_v1`), audit actions/categories, reset-to-default-via-baseline, high-risk acknowledgment, and `edit_platform_settings` gating are unchanged; no new permission keys; no enforcement/notifications/SSO/SCIM/providers/backend/server RBAC; M5 not started. Detail in [`docs/platform-operations-security-history.md`](docs/platform-operations-security-history.md). **Pending acceptance.**
...
-   **Phase 1.2E** — Domains Control Center UX Maturity — M1 Split-Pane **(accepted)** · M2 DNS/SSL/Security Workspaces **(accepted)** · M3 Platform Settings Control Center Layout **(accepted)** · M4 Default Baseline Manager + Change Review Center Polish **(done — pending acceptance)** · M5 not started
