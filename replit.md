...
-   **Shipping Provider Configuration**: Centralized configuration system accessible from Settings → Shipping and directly from the Shipping Center's Provider & Operations section. Supports EasyPost, Shippo, and ShipStation with credential configuration (masked display), environment selection, connection testing, and active provider selection, with state persisted in `sessionStorage`. All configuration actions are gated by `manage_shipping_settings` sub-permission and respect preview mode.
