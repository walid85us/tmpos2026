...
-   **Returns Portal / Reverse Logistics (Phase 2)**: First-class returns management module with its own dedicated route (`/returns`), sidebar entry, and permission domain (`returns`). Key architecture:
    ...
    -   **Source Entry Points**: Returns can be initiated from invoices, repair tickets, shipments, RMAs, or walk-in context. Source type and source number provide clear linkage.
    -   **Shipment Integration**: `originalShipmentId` links to the outbound shipment; `returnShipmentId` links to the return shipment. Return shipments use the existing `Shipment` type with `returnInfo.isReturn = true`. Provider/manual mode rules preserved — return shipments follow the same Shipping Center discipline as outbound shipments.
