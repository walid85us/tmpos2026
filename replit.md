...
-   **Latest work**: Phase 1.2E **Domains Control Center UX Maturity — Milestone 1 (Split-Pane Control Panel)** — presentation/layout refactor only of the System Owner Domains page: the table + slide-over becomes a split-pane control panel — left = grouped, scrollable **Domain Portfolio** (roots as parents with nested subdomains; Platform / Unlinked-Legacy / Other safety-net groups; muted "Context" rows for filtered-out parents of matching children; no count drift), right = a persistent, sticky **Domain Control Panel** (extracted `DomainControlPanel`) with a strong empty state; the same component still renders inside the existing mobile slide-over. No new derivations, permission keys, persistence, audit changes, real DNS/SSL/registrar, or Platform Settings changes; M2–M5 not started. Detail in [`docs/platform-operations-security-history.md`](docs/platform-operations-security-history.md). **Pending acceptance.**
