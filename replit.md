...
-   **Latest work**: Phase 1.2E **Domains Control Center UX Maturity — Milestone 2 (DNS / SSL / Security Workspaces)** — builds on the M1 split-pane panel by tabbing the right-pane `DomainControlPanel` into **Overview / DNS / SSL/TLS / Security / Help** workspaces. **All mutations stay in Overview** (single mutation surface — no new permission/audit semantics); the other tabs are informational/copy/guidance only. New rule-based, truth-labeled derivations in `platformOpsDomains.ts` (DNS/SSL readiness explanations, `deriveDomainSslWorkspace`, `deriveDomainTroubleshooting`, propagation steps, copy-all formatter) are computed **in-component for the selected record only** — not added to the shared signal — so posture/list/no-drift stay untouched. Added a dev-only (`import.meta.env.DEV`) no-drift console assertion. No real DNS/SSL/registrar/DNSSEC/provider; Platform Settings untouched; M3–M5 not started. Detail in [`docs/platform-operations-security-history.md`](docs/platform-operations-security-history.md). **Pending acceptance.**
...
-   **Phase 1.2E** — Domains Control Center UX Maturity — M1 Split-Pane **(accepted)** · M2 DNS/SSL/Security Workspaces **(done — pending acceptance)** · M3–M5 not started
