...
-   **Latest work**: Phase 1.2F **Strategic Replacement — "Domains" → "Tenant Web Address"**. **Presentation/positioning + one new pure helper; non-destructive over the M0 model.** Repositions the user-facing System Owner module away from a registrar/DNS "Domains" control panel toward **managing each tenant's platform web address** (`{tenant.subdomain}.repairplatform.com`). New pure helper `src/owner/tenantWebAddress.ts` derives, from `tenant_domains_v1` + tenant slug, each tenant's primary web address, customer-facing links (copyable), external-redirect guidance, a status (`active`/`reserved`/`needs_setup`/`disabled`/`external_only`) + next action + setup checklist, and truth/status/kind label maps — **`WEB_ADDRESS_LIVE_HOSTING=false`** so "Open" is disabled (Future/Not active) while Copy is always allowed. `DomainsPage.tsx` UI fully rewritten into a **Tenant Web Address** dashboard: 6 summary cards (Total / Active Platform Subdomains / Needs Setup / Disabled / External Redirects / Custom Domain Future), a web-address table (`table-fixed` colgroup, customer-link copy chips), saved views (all/active_platform/needs_setup/disabled/external_redirects/custom_future), Tenant/Status/External-redirect filters with visible clearable chips + Clear All, a slide-over **Web Address Overview** (Primary / Customer Links / External Redirect Guidance / Setup Checklist / Manage / Recent Activity / collapsed **Advanced Custom Domain Support** still backed by the M0 `deriveDomainControlPanelOverview`), and a Help surface. **No-drift**: ONE `webAddresses` signal array + ONE `matchesWebAddressFilter` predicate feed cards, table, and views. Sidebar (`OwnerLayout`), permission **display** labels (`platformPermissionsConfig` — group + View/Manage Tenant Web Address), and the Command Center quick-link were relabeled, all **without changing permission resolver/ids, storage, audit, the `tenant_domains_v1` store, the dormant M0 DNS/SSL/registrar helpers, the Add flow, or `?domain=`/`?status=` deep-linking**. No real DNS/SSL/registrar/hosting. Typecheck stable at the 12 pre-existing baseline errors (DomainsPage clean). **Done — pending acceptance; Phase 1.2/1.2E remain not accepted; Phase 1.3 not started.** **Table simplification correction (presentation-only, post-QA #7):** the web-address list was trimmed to scan-level columns — **Tenant · Platform Web Address (compact copy) · Status · External Website/Redirect · Customer Links · Next Action · Actions (Open Overview + Copy Main URL)**. The full **Main App URL** column and per-link URL cells were removed; Customer Links now shows a compact "N links available" + short chips (Portal/Book/Track/Pay/Mail-In), with full URLs and copy/open retained only in the selected overview/drawer. Same `deriveTenantWebAddresses` array + `matchesWebAddressFilter` predicate (no-drift), saved views, filters, chips, Clear All, search, permissions, and audit unchanged. **Final list simplification correction (presentation-only):** the table is now **one row per tenant/platform web address** — a tenant with both a platform subdomain record and a sibling external/redirect record is collapsed into a single visual row via a derived `tableRows` grouping memo over the filtered `visible` set (Pass 1 = one row per platform web address; Pass 2 = attach externals to that tenant's first platform row, or group external-only tenants into one row; two distinct platform web addresses stay two rows). Columns trimmed to **Tenant · Platform Web Address (inline copy icon) · Customer Links · External Website · Status · Actions**; the **Next Action column was removed** (verbose guidance lives only in the overview), the stacked Actions buttons collapsed to a **single "Open"**, and copy moved to an **inline icon beside the URL** (no large "Copy Main URL" button). The External Website cell is a clickable button that opens that external record's overview (detail access preserved without a duplicate row); when a tenant has multiple external records each additional one renders as its own clickable chip so every grouped external stays openable, and external-only rows show an "External Only" badge. **Final minimal-table correction (presentation-only):** the table was reduced to the four scan-level columns **Tenant · Platform Web Address (inline copy icon) · Status · Actions** — the **Customer Links** and **External Website** columns were removed (those details remain in the selected overview), and the action button was **renamed "Open" → "Manage"**. The tenant-grouping `tableRows` memo is unchanged (still one row per tenant/platform web address, external-only rows kept + manageable, "External Only" badge retained); the now-unused `CUSTOMER_LINK_SHORT` map was deleted. Because the External Website column is gone, the selected overview's "External website / redirect guidance" section now also lists the tenant's **sibling external/redirect records** (`selectedSiblingExternals`, derived from the flat `webAddresses` array) each with a **Manage** button, so every grouped external record stays reachable/manageable from the Manage flow (no record orphaned). The flat `webAddresses` array + `matchesWebAddressFilter` still drive all cards/saved-view counts (grouping is table-only), and saved views, filters, chips, Clear All, search, the selected overview, permissions, audit, and `?domain=`/`?status=` deep-linking are unchanged. **Manage Drawer UX Polish (presentation-only):** the selected slide-over (`WebAddressOverview`) was reorganized into a clean **Tenant Web Address Workspace** — a header (tenant name + platform web address with inline copy + kind/status chips) with **Copy Link** and disabled **Open Site (Future)** primary actions, then **Customer Access Points** (Main App + Portal/Book/Track/Pay/Mail-In as one scannable list via a new `AccessPointRow`, each with name · path preview · truthful status · copy · open-if-safe), **External Presence** (external website or empty state, redirect target, guidance, sibling-external Manage links, "Custom domain support: Future / Support-Assisted"), **Configuration Progress** (the setup checklist) and **Recent Activity**. The gated/audited lifecycle Manage controls and the collapsed read-only **Advanced Custom Domain Support** reference panel are preserved as-is. No model/storage/permission/audit/table changes; no DNS/SSL/registrar control-panel language reintroduced; old `CustomerLinkRow` replaced by `AccessPointRow`. **Final Drawer Product-Language Cleanup (presentation/wording-only):** the drawer's last domain-control-panel vocabulary was repointed to Tenant Web Address language without touching the model/storage/permissions/audit or raw status values. Manage controls now read in TWA vocab: the status select exposes only operator statuses **Active / Needs Setup** (via `MANAGE_STATUS_OPTIONS` + a `manageSelectValue` helper that maps any non-`verified` raw status down to "Needs Setup"), with the `disabled` state kept in its separate read-only "Disabled" box; button "Mark Pending"→"Mark Needs Setup"; raw `RAW_STATUS_LABELS` kept only for the advanced "Raw status" deep-link filter chip. Recent Activity uses a new display-only `ACTIVITY_LABELS`/`activityLabel()` map (raw audit keys + legacy seeded history labels → "Platform/Web Address Created", "Web Address Status Changed/Disabled/Re-enabled", "External Website Recorded/Reviewed", "Security State Updated") — audit storage and event IDs untouched. Metadata footer "Verified"→"Status reviewed", "Last checked"→"Last reviewed". The **Advanced Custom Domain Support** panel was de-technicalised: the DNS / SSL/TLS / Email DNS / Security readiness grid (and `deriveDomainControlPanelOverview`/`AdvCell`/the ADV label maps in `DomainsPage.tsx`) were removed in favor of the support-assisted explanation ("Custom domain hosting is not self-serve in this phase…") plus the existing truth labels; the gated, audited manual SSL-state control is kept (the M0–M2 helper foundation remains intact in `platformOpsDomainModel.ts`/`platformOpsDomains.ts`). Configuration Progress completed items no longer use strikethrough (green check + readable muted text). Typecheck stable at the 12 pre-existing baseline errors (DomainsPage clean).
-   **Prior work**: Phase 1.2F **Domain Control Panel Architecture Reset — Milestone 2 (Root/Subdomain Overview Workspace)**. **UI-only over the M0 model.** Rebuilt the selected-domain control panel's **Overview** tab to run off `deriveDomainControlPanelOverview` (the same M0 derivations that drive the M1 portfolio row), so the Overview and the portfolio row can never disagree on DNS/SSL/Email/Security/Risk/Next Action (no-drift extended from table to detail). Adds a header readiness-badge row (DNS/SSL/Email/Security + truth label), **4 manual readiness summary cards** (DNS / SSL / Email DNS / Security, each with status + explanation + next action + truth label), risk assessment, related-domain relationships (root→subdomains with lifecycle badges, subdomain→parent root, platform context), action checklist, and a date-granular Recent Activity section. **No new permission keys, no model/storage/seed/audit changes, no real DNS/SSL/registrar.** DNS/SSL/Security/Help tabs keep existing behavior (M3+ not built); Add-Domain, `?domain=`/`?status=` deep-linking, lifecycle/status/SSL/disable/re-enable, and all non-regression areas preserved. Typecheck stable at the 12 pre-existing baseline errors (DomainsPage clean). **M2 done — pending acceptance.**
-   **Prior work**: Phase 1.2F **Domain Control Panel Architecture Reset — Milestone 1 (Domain Portfolio Dashboard + Portfolio Table)**. UI-only over the M0 model — replaces the rejected 1.2E split-pane Domains landing with a **Domain Portfolio** dashboard: header + truth labels, **6 summary cards** (Total / Root·Sub / Pending·Failed / SSL Needs Attention / Email DNS Incomplete / Security Attention), a full-width **portfolio table** (Domain, Type, Tenant, Registrar, DNS, SSL, Email DNS, Security, Renewal·Expiry placeholder, Risk, Next Action, Actions), search, per-dimension filters, saved views, visible clearable chips + Clear All + truthful empty state. Row click opens the **unchanged `DomainControlPanel`** as a slide-over. **Locked no-drift**: one `portfolioSignals` array + one `matchesPortfolioFilter` predicate drives the table, `countWithFilters`, `countView`, and every clickable card. NO model/storage/audit/permission changes; NO real DNS/SSL/registrar; Add-Domain flow, `?domain=`/`?status=` deep-linking, audit, and `view_domains`/`manage_domain_lifecycle` gating preserved. Dead split-pane components removed. Architect review PASS; typecheck stable at the 12 pre-existing baseline errors. **Phase 1.2 / 1.2E remain not accepted; Phase 1.3 not started.** **Layout overflow correction done/pending acceptance** — removed the portfolio table's nested vertical/horizontal scroll (now `table-fixed` with a `colgroup`, compact wrapping cells, page-level scroll; horizontal scroll only as a sub-`xl` fallback) and made the existing domain-detail tab strip wrap instead of scroll horizontally. Layout-only; no M1 logic, columns, or behavior changed. **Table readability/density correction done/pending acceptance** — presentation-only: registrar cell now renders name + short muted context (full string in tooltip) via a `splitRegistrar` helper instead of wrapping the long seed string; Actions column widened and relabeled "Manage"→compact "Open" so it is never clipped; Renewal cell shows date-or-em-dash with a single compact "Auto-renew future" pill; 12 column widths rebalanced. No model/storage/permission/audit/column changes.
-   **Prior work**: Phase 1.2F **Milestone 0 (Domain Object Model Foundation)**. **Model/helper/storage/seed only — NO UI, NO Platform Settings, NO real integrations/automation.** Added `DnsRecord` (intended-state zone mirror), `DomainRegistrarInfo`, and a manual SSL/security overlay (`DomainSecurityRecord`) to `src/owner/mockData.ts` with mixed-state seed data; added deterministic helpers + sibling sessionStorage stores (`tenant_dns_records_v1`, `tenant_domain_registrar_v1`, `tenant_domain_security_v1`) in `src/owner/platformOpsDomainModel.ts` (DNS zone, Email DNS, SSL view, registrar/security/portfolio/overview/troubleshooting/audit-summary). Base `tenant_domains_v1` store NOT migrated; existing derivations reused (no drift); no new permission keys; future audit-action constants defined but NOT emitted.
...
-   **Phase 1.2F** — Domain Control Panel Architecture Reset → **Strategic Replacement ("Domains" → "Tenant Web Address")** **(in progress)** — M0 Domain Object Model Foundation **(done — pending acceptance; model/helper/storage/seed only, no UI)** · M1 Domain Portfolio Dashboard + Portfolio Table **(done — pending acceptance)** · M2 Root/Subdomain Overview Workspace **(done — pending acceptance)** · **Strategic Replacement to Tenant Web Address** **(done — pending acceptance; presentation/positioning + new pure `tenantWebAddress.ts` helper, non-destructive over the M0 model, dormant DNS/SSL/registrar helpers retained)**
