...
-   **Add-on Lifecycle & Archive Protection Rule (locked)**:
    -   **Four lifecycle states.** Every Add-on Catalog item has a `governanceStatus` of `draft`, `active`, `disabled`, or `archived`. The PM `lifecycle` is mirrored (draft → `draft`, active → `active`/`beta`/etc., archived → `archived`).
    -   **Default-Draft on create.** New add-ons created in the Add-on Catalog default to `draft`. Draft add-ons are **internal only**: hidden from every tenant surface, never offered as Trial / Paid Override, and never enable their linked feature even when the matrix toggle is on for the tenant's plan. Promoting a Draft add-on to Active emits an `addon_activated` audit entry.
    -   **Feature-matrix registration rule (no duplicates).** Every new add-on must reference a `linkedFeatureId` that exists in `featureMatrix`. The Add-on modal exposes an inline "+ New" register action that scaffolds a `featureMatrix` row (slug id, `source: 'custom'`, `lifecycle: 'implemented'`, all `planAvailability` false) and selects it. If the slug already exists, the existing row is reused — no duplicates. Registration emits a `feature_registered_for_addon` audit entry. The Save button is disabled until a linked feature is chosen.
    -   **Draft tenant visibility.** `resolveTenantFeature` step (3) returns `disabled_by_plan` with no `addOn` surface when the linked add-on is `draft`, even when the feature is in the plan baseline. `isAddOnOfferable` returns `true` only for `active`, so `listAvailableAddOnsForTenant` already excludes draft. The Tenant Features row visibility filter additionally hides any feature row whose linked add-on is `draft`.
    -   **Archive dependency categories.** Archive is **blocked** while ANY of the following are true: (1) the linked feature is included by plan in any active plan in the Plans & Features Matrix; (2) the add-on has any `compatiblePlans` selected; (3) there are non-revoked tenant overrides referencing the add-on (by `addOnId`) or its linked feature (by `featureId`) of type `trial`, `overridden`, `paid_override`, `addon`, or `pending_payment`.
    ...
    -   **Audit trail.** All add-on lifecycle transitions are auditable via `commercialAuditLog` with actions: `addon_created`, `addon_created_draft`, `addon_updated`, `addon_default_price_changed`, `addon_status_changed`, `addon_activated`, `addon_archive_blocked`, and `feature_registered_for_addon`.
