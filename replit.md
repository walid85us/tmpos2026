...
-   **Add-on Runtime Linkage & Delete/Archive Matrix Behavior (locked)**:
    ...
    -   **Archived standalone capability rows.** When an add-on is transitioned to `archived` and a `cap_<addonId>` row exists in the matrix, that row is locked read-only on the Plans & Features Matrix: plan toggles render as a lock icon, the lifecycle dropdown is disabled, and an "Add-on Archived" badge appears on the row. The lock is emitted exactly once at archive time as `standalone_capability_row_archive_locked`. Restoring the owning add-on from the catalog re-enables editing.
    -   **Show archived/deprecated filter.** The Plans & Features Matrix Features tab hides archived/deprecated rows by default — including standalone-cap rows whose owning add-on has been archived — so the matrix surfaces only live rows. The "Show archived/deprecated" toggle in the tab header re-introduces them for audit / restore workflows.
