...
-   **SLA Automation Backfill**: Manual, confirmed, itemized, duplicate-safe action that lets an authorized operator apply an existing SLA-trigger automation rule to existing matching shipments. The default future-only behavior is unchanged — rules still fire automatically only on new SLA events. The backfill is opt-in per rule via an "Apply to Existing" button on the rule card. Three-phase modal: (1) confirm — restates rule, scope, and safe-action disclosure; (2) preview — dry scan of current shipments grouped into will-apply / already-applied / no-current-SLA-state / did-not-match counts; (3) results — itemized per-shipment outcomes (`applied` / `already_applied` / `skipped` / `not_matched` / `failed`) with target labels, applied actions, and reasons. Supported triggers: `sla_at_risk`, `sla_overdue`, `sla_missed`, `sla_paused`, `sla_delay_reason_added`. `sla_resumed` is intentionally excluded (a one-off transient with no current state to backfill against). Matching reuses the live `evaluateRule` engine path with a synthesized per-shipment `AutomationTriggerContext` derived from the current `slaSummary` so the same condition language decides matching as during normal events. Execution is delegated to `runAutomation`, so the engine's safe-action whitelist (only `add_flag`, `add_internal_note`, `mark_review_needed`, `mark_ready_for_batch`, `set_priority` actually mutate; pre-action guardrail actions are silently skipped) remains the single source of truth — backfill cannot change shipment status, purchase labels, or perform irreversible carrier operations. Duplicate safety: each successful application records a stable backfill key `${ruleId}|${slaTargetType}` on the shipment in `slaAutomationBackfillKeys`; re-running the same backfill reports already-applied entries instead of double-applying. Audit: every applied shipment writes a global `automationLogs` entry tagged `triggeredBy: 'backfill'` and a shared `backfillRunId` (filterable in execution history); the rule itself stores up to the last 10 `backfillRuns` (counts + per-shipment outcomes + actor + timestamp) for at-a-glance "last backfill" line on the card; an internal note "SLA automation backfill applied: <ruleName> by <user>" is also appended to each affected shipment. Plan + permission gating: button visible only when the new `run_shipping_automation_backfill` sub-permission is granted AND both `shipping_automation_rules` and `shipping_sla_optimization` plan features are live AND writes are not blocked AND the rule is enabled and uses a backfillable SLA trigger. Defense-in-depth: all gates are re-checked at execution time; if any becomes false between scan and run the candidates are itemized as `failed` with a clear reason rather than silently skipped. **Out of scope:** Carrier Scorecards, Customs / HS code automation, Insurance recommendations, predictive SLA forecasting, scheduled / recurring backfills, automation-engine redesign, retroactive purchase or status mutations.
