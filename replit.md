...
-   **Shipping Center**: Centralized fulfillment module for managing shipments across all source documents, supporting various shipment types. Features include shipment listing with filters, detailed views, create/edit modals with package management, status transition workflows, tracking event timelines, carrier/service level configuration, cost tracking, and integration for data pre-population. Includes a provider adapter layer for external shipping carriers and a runtime mode architecture. Key features include:
    ...
    -   **Origin + Destination Address Validation**: Both addresses are independently validated. `Shipment.originAddressValidation` (mirrors `addressValidation` for destination) holds the validation result. `handleValidateAddress(shipmentId, side)` accepts `'origin' | 'destination'`; the detail panel exposes separate `Validate Origin` and `Validate Destination` buttons. `getRatePrerequisites` and `getLabelPrerequisites` both require `isOriginAddressAccepted(shipment) && isAddressAccepted(shipment)` before fetching rates or purchasing labels. Editing either address clears the corresponding validation result so stale validations cannot mask new typos. The detail panel renders two separate badges ("Origin Addr" / "Dest Addr") with their own validated/corrected/failed status pills.
    -   **Tracking Number Visibility**: Tracking numbers in tables (Bulk Sync eligibility list, Bulk Sync results) render in full with `break-all` wrapping rather than the previous 16-char truncation, so operators can copy the complete carrier reference without opening the detail panel.
...
-   **Returns Portal / Reverse Logistics (Phase 2)**: First-class returns management module with its own dedicated route (`/returns`), sidebar entry, and permission domain (`returns`). Key architecture:
    ...
    -   **Return/Shipment Lifecycle Synchronization**: Return status does NOT advance to "Label Created" on return shipment creation (shipment starts as Draft). A `useEffect` auto-sync watches the linked shipment's status and advances the return status accordingly: Shipment "Label Created"/"Packed" → Return "Label Created"; Shipment "Dispatched"/"In Transit" → Return "In Transit"; Shipment "Delivered" → Return "Delivered" (Awaiting Receipt). Auto-sync only advances forward (never regresses). When the shipment is still Draft, the return detail shows an amber notice directing the operator to the Shipping Center for label purchase. **Manual button suppression**: when a linked return shipment exists, the manual `Label Created`, `In Transit`, and `Delivered` transition buttons are hidden in the return detail modal — the shipping leg is exclusively driven by shipment auto-sync to prevent divergence. An indigo info banner explains "Shipping-leg statuses are auto-synced from the linked shipment". When no linked shipment exists (rare manual workflow), all manual buttons remain available.
