...
-   **Shipping Center Pass 6 Fixes**: (a) Settings tab renders ShippingProvidersPage inline within ShippingCenter (no navigate-away) via `embedded` prop. (b) Add Event button disabled for all post-dispatch statuses using `isPostDispatch()` guard. (c) Address autocomplete uses Nominatim/OpenStreetMap for real street-level suggestions with debounced async requests, local zip-code fallback, and source badges (OSM vs local). (d) Seed shipments include PDF-format `LabelArtifact` entries for demo purposes.
