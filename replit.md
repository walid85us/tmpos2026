...
- **Point of Sale (POS)**: A full-featured POS system including cart management, payments, repair intake, held orders, tax calculation (8.25%), discount application, customer management (inline creation, walk-in), and points redemption.
...
- **CRM**: Customer list, profiles, and new customer creation.
...
- **Data Flow**: `StoreLocalState.tsx` manages shared local mock state for customers and stock items. `StockItem` has `cost` and `price` (sell price) fields. Seeded with 5 real items. Both Add Stock forms (dashboard and POS) capture sell price. Customers.tsx merges shared state customers into its local rich-format list.
- **Dashboard Quick Actions**: Quick Intake opens a repair intake modal directly on the dashboard (not navigation). Print Label uses inventory-backed search with type-aware preview. Scan QR searches inventory and navigates to POS with `addToCart` location state. Add Stock shows item verification details on success.
- **POS Location State**: Supports `autoQuickCheckIn`, `openHeldOrders`, `autoRepairItem` (pre-filled repair item from dashboard), and `addToCart` (scanned item). Repair Intake modal has visible X close button.
- **POS Catalog**: Add Item catalog merges hardcoded base items with shared stock items (deduped by name).
