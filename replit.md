...
-   **Next planned phase**: Phase 1.3 — Platform Team Governance (server-side RBAC / PIM / PAM). **Milestone 0 (Current Access Inventory) is documentation-only and complete** — the authoritative inventory of platform feature groups / sub-permissions / sensitive flags / role defaults / resolver model and the future server-side enforcement tier classification (Tier 0–3) live in [`docs/phase-1.3-platform-access-inventory.md`](docs/phase-1.3-platform-access-inventory.md). Milestones 1–5 (governance model, directory/role-matrix UI, temporary access/PIM, access review + reason capture, Command Center/Audit integration) are **not started**.
